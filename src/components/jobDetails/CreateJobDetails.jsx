...
export default function CreateEmploymentDetails({ nav, header, postRequest, fetchEmployees }) {
    ...
    const handleChange = (e) => {
        ...
        setEmploymentDetailsForm((prevData) => ({
            ...prevData,
            [name]: name === "reportsTo" || name === "employeeId"? Number(value) : value,
        }));
    }

    const handleEmploymentDetailsSubmit = async (e) => {
        e.preventDefault();
        const employmentDetailsPostJson = await postRequest("https://hris-qp6t.onrender.com/employmentdetail/create", employmentDetailsForm);
